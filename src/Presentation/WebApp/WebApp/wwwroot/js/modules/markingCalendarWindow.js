...
/** Стили блоков календаря для тёмной темы Webix. */
...
async function loadItems() {
    ...
    const result = await AuthService.makeAuthenticatedRequest("/api/marking-calendar");
    if (!result.result)
        throw new Error(result.error || LABELS.loadError);
...
/** Webix не принимает числа в options richselect. */
...
/** Дата в ответе Честного знака приходит как ДД.ММ.ГГГГ. */
...
/** Названия групп в ответе содержат HTML-сущности. */
