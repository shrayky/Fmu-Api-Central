...
{
    private readonly IUsersManagerService _manager;

    public UsersController(IUsersManagerService manager)
    {
        _manager = manager;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Post([FromBody] UserView data)
    {
    ...
    [HttpPut]
    public async Task<IActionResult> Put([FromBody] UserView data)
    {
    ...
    public async Task<IActionResult> ChangePassword(string id, [FromBody] ChangeUserPasswordRequest data)
    {
        var result = await _manager.ChangePassword(id, data.Password);
